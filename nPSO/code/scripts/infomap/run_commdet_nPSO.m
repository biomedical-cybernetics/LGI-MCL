...
for k = 1:length(kernels)
    if exist(['../../results/nuPSOv16_' kernels{k} '_infomap_NMI.mat'],'file')
        continue
    end
    display(kernels{k})
            ...
            for i3 = 1:size(matrices,3)
                for i4 = 1:size(matrices,4)
                    for i5 = 1:size(matrices,5)
                        time = tic;
                        for i6 = 1:size(matrices,6)
                            C = length(unique(comm_real{i1,i2,i3,i4,i5,i6})); % number of communities
                            
                            % community detection
                            comm = infomap(matrices{i1,i2,i3,i4,i5,i6}, kernels{k}, ['nPSO_infomap_' num2str(i6)], 'C:\cygwin64\bin');
    ...
    save(['../../results/nuPSOv16_' kernels{k} '_infomap_NMI.mat'], 'NMI')
end