...
% coords - polar coordinates (radius, theta) of the network nodes
...
elseif isscalar(distr) && distr > 0
    ...
    coords(:,2) = mod(random(gmd,N),2*pi);
    [~,comm] = min(pi - abs(pi-abs(repmat(coords(:,2),1,C)-repmat(mu,N,1))),[],2);
else
    ...
    mu = distr{3};
    C = length(mu);
    coords(:,2) = mod(angles(randsample(length(angles),N,true,angles_prob)),2*pi);
    if iscolumn(mu)
        mu = mu';
    end
    [~,comm] = min(pi - abs(pi-abs(repmat(coords(:,2),1,C)-repmat(mu,N,1))),[],2);
...
for t = 2:N
    ...
    coords(1:t-1,1) = beta .* (2*log(1:t-1)) + (1-beta)*2*log(t);
...
if isempty(comm)
    colours = compute_similarity_colours(coords(:,2));
else
...
[cart_coords(:,1),cart_coords(:,2)] = pol2cart(coords(:,2),coords(:,1));
...
% (radial coord, angular coord). The resulting similarities are stored in d.
%
% INPUT
...
if ~isreal(d)
    d(imag(d)~=0) = abs(XI(1)-XJ(imag(d)~=0,1));
end
