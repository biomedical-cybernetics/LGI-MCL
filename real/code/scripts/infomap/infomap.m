function comm = infomap(x, kernel, fileprefix, exe_path)

currdir = [pwd '\'];
fileinput = [fileprefix '.txt'];
fileoutput = [currdir fileprefix '.tree'];
...
% run infomap
command = [exe_path '\Infomap.exe ' fileinput ' ' currdir ' -N 10 --silent'];
system(command);

% read output
comm = parse_infomap_tree(fileoutput);
delete(fileinput)
delete(fileoutput)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function comm = parse_infomap_tree(filename)

fid = fopen(filename);

% parse number of nodes and levels
% note: last level (leaves) is discarded
line = fgetl(fid);
C = strsplit(line, '->');
C = strsplit(strtrim(C{2}));
n = str2double(C{1});
m = str2double(C{end-1})-1;
comm = zeros(n,m);
...
line = fgetl(fid);
for i = 1:n
    l = length(strfind(line, ':'));
    C = strsplit(line, {':', ' '});
    node = str2double(C(end));
    for j = 1:l
        curr(j) = str2double(C{j});
        if any(curr(1:j)~=prev(1:j))
            ids(j) = ids(j)+1;
        end
        comm(node,j) = ids(j);
    end
