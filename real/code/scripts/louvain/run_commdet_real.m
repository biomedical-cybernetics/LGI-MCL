...
for i = 1:length(networks)
    ...
    for k = 1:length(kernels)
        if exist(['../../results/' networks{i} '_' kernels{k} '_louvain_NMI.mat'],'file')
            continue
        end
        
        % community detection
        comm = louvain_R(x, kernels{k}, ['real_louvain_' num2str(k)], 'C:\Users\mluser\Documents\R\R-3.4.3\bin\x64');
    ...
    for k = 1:length(kernels)
        NMI = NMI_kernels(k);
        save(['../../results/' networks{i} '_' kernels{k} '_louvain_NMI.mat'], 'NMI')
    end
end