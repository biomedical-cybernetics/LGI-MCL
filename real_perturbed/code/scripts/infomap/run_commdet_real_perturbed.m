...
for i = 1:length(networks)
    for p = 1:length(perturb)
        ...
        for k = 1:length(kernels)
            if exist(['../../results/' networks{i} '_' perturb{p} '_' kernels{k} '_infomap_NMI.mat'],'file')
                continue
            end
            ...
            for j = 1:length(matrices)
				...
				% community detection
                comm = infomap(matrices{j}, kernels{k}, ['realpert_infomap_' num2str(j)], 'C:\cygwin64\bin');
            ...
            save(['../../results/' networks{i} '_' perturb{p} '_' kernels{k} '_infomap_NMI.mat'], 'NMI')
            time = round(toc(time));
            fprintf(' [%.0fs]\n', time)
        end
    end
